...
public class TextureCreatorWindow : EditorWindow
{
    ...
    int perlinOffsetX;
    int perlinOffsetY;
    bool alphaToggle = false; // For transperancy. 
    bool seamlessToggle = false;
    bool mapToggle = false; // Remap the coulor values to be between 0 and 1. 
    ...
    // Remember this method will run everytime we recomplie this script.
    void OnEnable()
    {
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    if(seamlessToggle)
                    {
                        ...
                        float noise00 = Utilis.fBM((x + perlinOffsetX) * perlinXScale, (y + perlinOffsetY) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
                        float noise01 = Utilis.fBM((x + perlinOffsetX) * perlinXScale, (y + perlinOffsetY + h) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
                        float noise10 = Utilis.fBM((x + perlinOffsetX + w) * perlinXScale, (y + perlinOffsetY) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
                        float noise11 = Utilis.fBM((x + perlinOffsetX + w) * perlinXScale, (y + perlinOffsetY + h) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale; 
                    ...
                    {
                        pValue = Utilis.fBM((x + perlinOffsetX) * perlinXScale, (y + perlinOffsetY) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
            ...
            if (mapToggle)
            {
                for (int y  = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        ...
                        colValue = Utilis.Map(colValue, minColor, maxColor, 0, 1);
                        pixCol.r = colValue;
                        pixCol.g = colValue;
                        pixCol.b = colValue;
                        pTexture.SetPixel(x, y, pixCol);
